...
inv_df = pd.read_excel(INV_XLS, sheet_name="Sheet0", dtype=str)
inventory = (
    inv_df['query_id']
    .dropna()
    .str.strip()
...
excluded = wide.filter(regex=r"^(Pand\d+|Entry_|HOG($|_))").columns
entry_cols = [c for c in wide.columns if c not in set(excluded)]
